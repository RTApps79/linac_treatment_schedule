...
(() => {
  ...
  const VERSION = "v10.0.0 (2025-12-21)";
  ...
  // Static aperture cue + simple OAR blocking hint (non‑IMRT)
  ctx.strokeStyle = "rgba(0,255,120,0.55)";
  ctx.lineWidth = 2;
  ctx.strokeRect(ax, ay, bx - ax, by - ay);

  // Subtle "leaf" guides so it reads like an MLC view even for static fields
  ctx.strokeStyle = "rgba(255,255,255,0.06)";
  ctx.lineWidth = 1;
  const leafCount = 18;
  for (let i = 1; i < leafCount; i++) {
    const y = ay + (i * (by - ay)) / leafCount;
    ctx.beginPath();
    ctx.moveTo(ax, y);
    ctx.lineTo(bx, y);
    ctx.stroke();
  ...
  // Simulated shielding blocks (deterministic per field)
  this._drawStaticBlocks(ctx, ax, ay, bx, by, Number(field?.staticBlockPattern ?? 0));
}
// Crosshair
...
_drawStaticBlocks(ctx, ax, ay, bx, by, pattern) {
  const w = bx - ax;
  const h = by - ay;
  if (w <= 6 || h <= 6) return;
  ...
  const ptn = (Number.isFinite(pattern) ? pattern : 0) % 4;

  if (ptn === 0) {
    // Right-side shielding slab (lung/heart-style)
    ctx.beginPath();
    ctx.moveTo(bx - 0.30 * w, ay + 0.08 * h);
    ctx.lineTo(bx,          ay + 0.18 * h);
    ctx.lineTo(bx,          by - 0.18 * h);
    ctx.lineTo(bx - 0.34 * w, by - 0.08 * h);
    ctx.closePath();
    ctx.fill(); ctx.stroke();
  } else if (ptn === 1) {
    // Lower-left triangle
    ctx.beginPath();
    ctx.moveTo(ax, ay + 0.62 * h);
    ctx.lineTo(ax + 0.44 * w, by);
    ctx.lineTo(ax, by);
    ctx.closePath();
    ctx.fill(); ctx.stroke();
  } else if (ptn === 2) {
    // Diagonal wedge across the top
    ctx.beginPath();
    ctx.moveTo(ax + 0.08 * w, ay);
    ctx.lineTo(bx, ay);
    ctx.lineTo(bx - 0.18 * w, ay + 0.26 * h);
    ctx.lineTo(ax + 0.22 * w, ay + 0.20 * h);
    ctx.closePath();
    ctx.fill(); ctx.stroke();
  } else {
    // Central notch / island
    const nx = ax + 0.54 * w;
    const ny = ay + 0.44 * h;
    const nw = 0.22 * w;
    const nh = 0.16 * h;
...
_renderBEV(ax, ay, bx, by, imrtLike, progressPct, elapsedMs, field) {
  ...
  // Clip to the circular view (Beam's Eye View)
  ctx.save();
  ctx.beginPath();
  ctx.arc(cx, cy, r, 0, Math.PI * 2);
  ctx.clip();

  // VMAT: subtly rotate the overlay during delivery based on gantry angle
  const tech = String(field?.technique || "").toUpperCase();
  const gDeg = Number(field?._gantryActualDeg);
  const shouldRotate = tech.includes("VMAT") && Number.isFinite(gDeg);
  const rotRad = shouldRotate ? (gDeg * Math.PI / 180) * 0.06 : 0; // subtle: 6% of gantry angle
  ...
  // Aperture / MLC
  if (imrtLike) {
    // Dynamic "leaf bar" cue (IMRT/VMAT)
    const n = 18;
    const barH = (r * 2) / n;
    ...
    // Static field: draw jaws + static block pattern
    const jaw = field?.jawPositions_cm || null;
    let ax2 = cx - r * 0.55;
    let ay2 = cy - r * 0.55;
    let bx2 = cx + r * 0.55;
    let by2 = cy + r * 0.55;

    if (jaw && [jaw.X1, jaw.X2, jaw.Y1, jaw.Y2].every(Number.isFinite)) {
      const jawMax = 20; // cm half-width for mapping
      const s = r / jawMax;
      ax2 = cx + jaw.X1 * s;
      bx2 = cx + jaw.X2 * s;
      ay2 = cy + jaw.Y1 * s;
    ...
    ctx.strokeStyle = "rgba(0,255,120,0.55)";
    ctx.lineWidth = 2;
    ctx.strokeRect(ax2, ay2, bx2 - ax2, by2 - ay2);

    // Leaf guides + simulated shielding
    ctx.strokeStyle = "rgba(255,255,255,0.06)";
    ctx.lineWidth = 1;
    const leafCount = 16;
    for (let i = 1; i < leafCount; i++) {
      const y = ay2 + (i * (by2 - ay2)) / leafCount;
      ctx.beginPath();
      ctx.moveTo(ax2, y);
      ctx.lineTo(bx2, y);
      ctx.stroke();
    }
    this._drawStaticBlocks(ctx, ax2, ay2, bx2, by2, Number(field?.staticBlockPattern ?? 0));
  }
...
function stableHash(str) {
  // Deterministic, simple hash (32-bit)
  let h = 2166136261;
  const s = String(str || "");
...
function computeDefaultCouchPlan(scenarioId, siteText) {
  const h = stableHash(String(scenarioId || ""));
  const s = String(siteText || "").toLowerCase();

  // Varian-ish looking defaults (cm for translations, degrees for rotations).
  let couchVrt = 24.0 + ((h >>> 0) % 30) / 10;   // 24.0–26.9
  let couchLng = 110.0 + ((h >>> 3) % 60) / 10;  // 110.0–115.9
  let couchLat = 100.0 + ((h >>> 6) % 40) / 10;  // 100.0–103.9
  let pitch = 0.0;
  let roll  = 0.0;
  let couchRtn = 0.0;

  if (s.includes("breast")) {
    couchLng = 130.0 + ((h >>> 3) % 40) / 10;    // 130.0–133.9
    couchLat =  99.0 + ((h >>> 6) % 30) / 10;    //  99.0–101.9
  } else if (s.includes("pelvis") || s.includes("prostate")) {
    couchLng = 120.0 + ((h >>> 3) % 50) / 10;    // 120.0–124.9
    couchLat = 100.0 + ((h >>> 6) % 30) / 10;    // 100.0–102.9
  } else if (s.includes("spine")) {
    couchLng = 105.0 + ((h >>> 3) % 40) / 10;    // 105.0–108.9
    couchLat = 100.0 + ((h >>> 6) % 30) / 10;    // 100.0–102.9
  } else if (s.includes("femur")) {
    couchLng = 115.0 + ((h >>> 3) % 50) / 10;    // 115.0–119.9
  }

  return { couchVrt, couchLng, couchLat, pitch, roll, couchRtn };
}

function normalizeJawPositions(field) {
  const jp = field?.jawPositions_cm || field?.jawPositions || field?.jawPositionsCm || null;
  if (!jp) return null;
  const X1 = Number(jp.X1 ?? jp.x1);
  const X2 = Number(jp.X2 ?? jp.x2);
  const Y1 = Number(jp.Y1 ?? jp.y1);
  const Y2 = Number(jp.Y2 ?? jp.y2);
  if (![X1, X2, Y1, Y2].every(Number.isFinite)) return null;
  return { X1, X2, Y1, Y2 };
}

function pickStaticBlockPattern(fieldName) {
  const n = stableHash(String(fieldName || "")) % 4;
  return n; // 0..3
}

function normalizeFields(rawFields, treatmentPlan, scenarioId) {
  const tp = treatmentPlan || {};
  const rx = tp.prescription || {};
  const siteText = tp.site || tp.treatmentSite || rx.site || "";
  const couchPlan = computeDefaultCouchPlan(scenarioId, siteText);

  const fields = (Array.isArray(rawFields) ? rawFields : []).map((f, idx) => {
    const out = { ...(f || {}) };

    // Normalize common display fields
    out.fieldId = out.fieldId || out.id || `F${idx + 1}`;
    out.fieldName = out.fieldName || out.name || `Field ${idx + 1}`;
    out.technique = out.technique || out.modality || out.type || "STATIC";

    // Dose / machine
    out.monitorUnits = Number(out.monitorUnits ?? out.MU ?? out.mu ?? 0);
    out.doseRate = Number(out.doseRate ?? 600);
    out.wedge = out.wedge || "None";

    // Bolus typically belongs to the prescription, not individual beams.
    if (out.bolus == null) out.bolus = (rx.bolus != null ? String(rx.bolus) : "None");

    // Geometry
    const jaw = normalizeJawPositions(out);
    if (jaw) {
        out.jawPositions_cm = jaw;
        out.jawX1 = jaw.X1;
        out.jawX2 = jaw.X2;
        out.jawY1 = jaw.Y1;
        out.jawY2 = jaw.Y2;
    ...
    // Provide realistic "plan" couch values (cm / degrees)
    out.couchVrt = Number(out.couchVrt ?? couchPlan.couchVrt);
    out.couchLng = Number(out.couchLng ?? couchPlan.couchLng);
    out.couchLat = Number(out.couchLat ?? couchPlan.couchLat);
    out.pitch = Number(out.pitch ?? couchPlan.pitch);
    out.roll = Number(out.roll ?? couchPlan.roll);
    out.couchRtn = Number(out.couchRtn ?? couchPlan.couchRtn);

    // Imaging identification
    const techLower = String(out.technique || "").toLowerCase();
    const nameLower = String(out.fieldName || "").toLowerCase();
    out.isImaging = Boolean(out.isImaging) ||
      Boolean(out.imagingModality) ||
      /(setup|imaging|cbct|kv|mv)/.test(techLower) ||
      /(setup|imaging|cbct|kv|mv)/.test(nameLower);

    // Static-field "block"/shielding cue pattern
    out.staticBlockPattern = pickStaticBlockPattern(out.fieldName);

    // Helpful for stable ordering if needed
    out._orderIndex = idx;

    return out;
  });

  // Ensure imaging shows first (more realistic pre-treatment workflow)
  const imaging = fields.filter(f => f.isImaging);
  const treatment = fields.filter(f => !f.isImaging);
  return [...imaging, ...treatment];
}
...
// ---------- Main controller ----------
  class LinacConsole {
    constructor({ scenario, file, cfg }) {
      ...
      const rawFields = Array.isArray(tp.treatmentFields) ? tp.treatmentFields :
        (Array.isArray(scenario?.treatmentFields) ? scenario.treatmentFields : []);
      this.fields = normalizeFields(rawFields, tp, this.scenarioId);
