...
(function () {
  ...
  function ensureUI() {
    ...
    $('#study-export').addEventListener('click', () => {
      ...
      window.StudyCapture.exportCSV({ includeOptional: true });
    });

    $('#study-export-audit').addEventListener('click', () => {
      if (!ensureStudyCaptureLoaded()) return;
      window.StudyCapture.exportAuditCSV();
    });
  ...
  function renderSetup() {
    ...
    content.innerHTML = `
      ...
      <div class="study-row study-row-3">
        <div class="study-field">
          <label>Mode</label>
          <select id="p_mode">
            <option value="study">Study (persist to browser)</option>
            <option value="demo">Demo (no persistence)</option>
          </select>
          <div class="study-hint" style="margin-top:6px">Demo mode is for practice/testing and will NOT write to localStorage.</div>
        </div>
        <div class="study-field">
          <label>App Version</label>
          <input id="p_appVersion" value="${safeStr(window.StudyCapture?.getVersion?.() || '')}" disabled />
        </div>
        <div class="study-field">
          <label>Export Schema</label>
          <input id="p_schemaVersion" value="${safeStr(window.StudyCapture?.getSchemaVersion?.() || '')}" disabled />
        </div>
      </div>
    ...
    const currentMode = window.StudyCapture?.getMode?.() || 'study';
    const modeEl = document.getElementById('p_mode');
  ...
  function saveParticipant() {
    ...
    // Apply mode selection (Study vs Demo)
    const modeSel = safeStr(document.getElementById('p_mode')?.value || 'study').trim().toLowerCase();
    if (window.StudyCapture && typeof window.StudyCapture.setMode === 'function') {
      window.StudyCapture.setMode(modeSel === 'demo' ? 'demo' : 'study');
    }
