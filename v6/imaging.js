...
(() => {
  ...
  class ImagingScreen {
    ...
    renderOverlayTransform() {
      const overlay = $("overlay-image");
      if (!overlay) return;
      const x = this.shift.lat * PX_PER_CM;
      const y = this.shift.vrt * PX_PER_CM;
      const pitchDeg = this.shift.pitch;
      overlay.style.transform = `translate(${x}px, ${y}px) rotate(${pitchDeg}deg)`;
    }
    ...
    async loadScenario() {
      try {
        ...
        // Default opacity
        const slider = $("overlay-opacity");
        if (overlayEl && slider) overlayEl.style.opacity = String(slider.value);
